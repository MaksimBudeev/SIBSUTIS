...
long long Shamir_decode (string code, string key, string output) {
	...
	uint64_t length;
	long long *dec = read_enc(code, &length);
	long long p, Db;

	ifstream file_key;
	file_key.open(key, ios::binary);
	file_key >> p >> Db;
	file_key.close();

	for(uint64_t i = 0; i < length; ++i) {
		dec[i] = powmod(dec[i], Db, p);
        //x4[i] = powmod(x3[i], Db, p);
	}

	write_dec(output, dec, length);
	delete []dec;
...
void El_Gamala_decoding (string code, string key, string output)
{
	...
	for(uint64_t i = 0; i < length; ++i) {
		dec[i] = (powmod(r, (p - 1 - Cb), p) * dec[i]) % p;
	}
